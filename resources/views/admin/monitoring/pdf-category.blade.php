...
<body>
    ...
    <p class="text-center"><strong>{{ $kategori->awal }} {{ $kategori->is_reverse == 0 ? 'Tidak' : '' }} {{ $kategori->akhir }}</strong> dari tanggal {{ date('d/m/Y', strtotime($t1)) }} sampai dengan {{ date('d/m/Y', strtotime($t2)) }}</p>
    <table class="bordered">
        ...
        <tbody>
            @if(count($monitoring_detail) > 0)
                @foreach($monitoring_detail as $key=>$md)
                <?php $m = $md->monitoring; ?>
                <tr>
